...
class DistanceTest {
    ...
    public List<List<Double>> minMaxNormalization(Car...cars) {
        List<Double> values = new ArrayList<>();

        for (Car car : cars) {
            values.add((double) car.getMileage());
            values.add((double) car.getYear());
            values.add((double) car.getPrice());
        }
        ...
        for (int i = 0; i < numCars; i++) {
            ...
            finalValues.add(normalizedValues.subList(startIndex, endIndex));
        }
    ...
    public void Manhattantest() throws Exception {
        List<List<Double>> l1 = minMaxNormalization(car1,car2,car3,car4,car5,car6);
